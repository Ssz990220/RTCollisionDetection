...
namespace RTCD {
    ...
    void addObj(T& s, bool loop) {
        ...
        std::unique_ptr<obstacle> bin1 = std::make_unique<RTCD::obstacle>(
            CONCAT_PATHS(PROJECT_BASE_DIR, "/models/curobo/bin_dense.obj"), pose, 1.0, loop);
        s->addObstacle(std::move(bin1));

        pose = Eigen::Affine3f::Identity();
        pose.translate(Eigen::Vector3f(0, 0.6, 0));
        std::unique_ptr<obstacle> bin2 = std::make_unique<RTCD::obstacle>(
            CONCAT_PATHS(PROJECT_BASE_DIR, "/models/curobo/bin_dense.obj"), pose, 1.0, loop);
        ...
        pose = Eigen::Affine3f::Identity();
        pose.translate(Eigen::Vector3f(-0.4, 0, 0));
        std::unique_ptr<obstacle> Face1 = std::make_unique<RTCD::obstacle>(
            CONCAT_PATHS(PROJECT_BASE_DIR, "/models/curobo/Face1.obj"), pose, 1.0, loop);
        s->addObstacle(std::move(Face1));

        pose = Eigen::Affine3f::Identity();
        pose.translate(Eigen::Vector3f(-0.4, 0, 0));
        std::unique_ptr<obstacle> Face2 = std::make_unique<RTCD::obstacle>(
            CONCAT_PATHS(PROJECT_BASE_DIR, "/models/curobo/Face2.obj"), pose, 1.0, loop);
        s->addObstacle(std::move(Face2));

        pose = Eigen::Affine3f::Identity();
        pose.translate(Eigen::Vector3f(-0.4, 0, 0));
        std::unique_ptr<obstacle> Face3 = std::make_unique<RTCD::obstacle>(
            CONCAT_PATHS(PROJECT_BASE_DIR, "/models/curobo/Face3.obj"), pose, 1.0, loop);
        s->addObstacle(std::move(Face3));
