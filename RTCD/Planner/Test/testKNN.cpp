...
bool test(const float * ref,
    ...
    // Elapsed time in ms
    double elapsed_time = std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::steady_clock::now() - begin).count();
    ...
    if (precision_accuracy >= min_accuracy && index_accuracy >= min_accuracy ) {
        printf("PASSED in %8.5f milliseconds (averaged over %3d iterations)\n", elapsed_time / nb_iterations, nb_iterations);
    }
    else {
        printf("FAILED\n");
    }
...
bool test(const float * ref,
    ...
    knn.downloadResult(test_knn_dist, test_knn_index);
...
int main(void) {
    ...
    // Parameters
    const int ref_nb   = NUM;
    const int query_nb = NUM;
    const int dim      = DIM;
    const int k        = KNN_K;
    ...
    RTCD::CUDABuffer refBuffer(ref_nb * dim * sizeof(float));
    refBuffer.upload(ref, ref_nb * dim);

    RTCD::KNN::KNN knn(ref, cudaMemcpyHostToDevice);

    // Compute the ground truth k-NN distances and indexes for each query point
    printf("Ground truth computation in progress...\n\n");
    if (!knn_c(ref, ref_nb, ref, ref_nb, dim, k, knn_dist, knn_index)) {
        free(ref);
	    free(query);
	    free(knn_dist);
	    free(knn_index);
    ...
    printf("TESTS\n");
    test(ref, ref_nb, ref, ref_nb, dim, k, knn_dist, knn_index, &knn_c,"knn_c",100);
    test(ref, ref_nb, ref, ref_nb, dim, k, knn_dist, knn_index, knn, "cudaKNN",100);
